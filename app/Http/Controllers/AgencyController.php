...
final class AgencyController extends Controller
{
    ...
    public function show(Request $request, string $module = 'dashboard'): mixed
    {
        ...
        return match ($route) {
            ...
            'clients' => $this->laravelPage('entity-list', array_merge($this->clientPage($options), ['rows'=>$this->agency->clients($request->query()),'filters'=>$request->query(),'filterOptions'=>$options]), $route),
            ...
            'projects' => $this->laravelPage('entity-list', array_merge($this->projectPage($options), ['rows'=>$this->agency->projects($request->query()),'filters'=>$request->query(),'filterOptions'=>$options]), $route),
            'tasks' => $this->laravelPage('entity-list', array_merge($this->taskPage($options), ['rows'=>$this->agency->tasks($request->query()),'filters'=>$request->query(),'filterOptions'=>$options]), $route),
            ...
            'saved_quotes' => $this->laravelPage('saved-quotes', ['title'=>'Saved Quotes','quotes'=>$this->quotes->savedQuotes($request->query()),'filters'=>$request->query(),'filterOptions'=>$this->quotes->savedQuoteFilterOptions()], $route),
    ...
    private function handlePost(string $route): void
    {
        ...
        try {
            ...
            $handlers = [
                ...
                'update_project'=>['projects.manage', fn()=>$this->agency->updateProject($_POST), 'projects', 'Project updated successfully.'],
                'update_task'=>['tasks.manage', fn()=>$this->agency->updateTask($_POST), 'tasks', 'Task updated successfully.'],
                'duplicate_task'=>['tasks.manage', fn()=>$this->agency->duplicateTask((int)$_POST['task_id']), 'tasks', 'Task duplicated successfully.'],
                'add_task_note'=>['tasks.manage', fn()=>$this->agency->addTaskNote((int)$_POST['task_id'],(string)($_POST['note']??'')), 'tasks', 'Task note added to the activity trail.'],
                'delete_task'=>['tasks.manage', fn()=>$this->agency->deleteTask((int)$_POST['task_id']), 'tasks', 'Task deleted successfully.'],
                ...
                'delete_employee'=>['team.manage', fn()=>$this->agency->deleteEmployee((int)$_POST['employee_id']), 'team', 'Team member and linked login permanently deleted.'],
    ...
    private function clientPage(array $o): array { return ['title'=>'Clients','subtitle'=>'Clients accepted and onboarded through approved quotes.','route'=>'clients','addLabel'=>'New Client','action'=>'create_client','columns'=>[['business_name','Business','client_link'],['name','Primary contact','text'],['email','Email','text'],['phone','Phone','text'],['package_name','Package','text'],['accepted_quote_value','Accepted quote','money'],['monthly_price','Monthly recurring','money'],['joined_at','Onboarded','date']], 'fields'=>[]]; }
